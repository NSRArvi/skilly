    ...
    return (
      <div className="bg-card border border-border rounded-2xl p-8 text-center text-muted-foreground space-y-3">
        ...
        <p className="text-xs max-w-md mx-auto">
          Post your first job to start receiving applications from talented professionals.
        </p>
  ...
  return (
    <div className="space-y-4">
      ...
      {jobs.map((job) => {
        ...
        const locationStr = locationParts.length > 0 ? locationParts.join(", ") : job.location_type || "Remote";
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
              <div className="space-y-1.5 flex-1 min-w-0">
                ...
                <div className="flex flex-wrap items-center gap-3 text-[11px] text-muted-foreground">
                  ...
                  <span className="font-semibold text-foreground">{salaryStr}</span>
                  {job.categories?.name && (
                    <span className="text-primary font-medium">{job.categories.name}</span>
                  )}
                </div>
              </div>
              ...
              <div className="flex items-center gap-2 flex-shrink-0">
                <Link href={`/jobs/${job.id}`}>
                  <Button variant="outline" size="sm" className="h-8 rounded-lg text-[11px] font-semibold gap-1">
                    <ExternalLink className="w-3 h-3" />
                    View
                  </Button>
                </Link>
                <Link href={`/jobs/${job.id}/edit`}>
                  <Button variant="outline" size="sm" className="h-8 rounded-lg text-[11px] font-semibold gap-1 text-primary hover:text-primary hover:bg-primary/10 border-primary/30">
